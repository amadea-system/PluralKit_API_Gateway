...
class Fronters:
    ...
    @staticmethod
    def get_by_hid(hid: str, authorization=None):
        fronters_json = api_get(f"/s/{hid}/fronters", authorization)
        fronters = Fronters(**fronters_json)
        fronters.json = fronters_json
        return fronters
...
class System:
    ...
    tz: str
    api_token: str
    fronters_cache_timestamp: float
    ...
    def __init__(self, id, created, name=None, description=None, tag=None, avatar_url=None, tz=None,
        ...
        self.api_token = None
        self.fronters_cache_timestamp = time.time()
        self.json = None

        self._fronters: Optional[Fronters] = None
        self._members: Optional[Members] = None
        self._fronter_decay_time = 15  # Seconds
    ...
    @property
    def fronters(self) -> Fronters:
        self.update_fronters()  # Updates self._fronters if needed
    ...
    def update_fronters(self, forced=False) -> None:

        if self._fronters is None or forced or time.time() - self.fronters_cache_timestamp > self._fronter_decay_time:
            # If we don't yet have a fronter object or if the fronters object we have is old.
            # Get new fronters object
            fronters_json = api_get(f"/s/{self.hid}/fronters", self.api_token)
            fronters = Fronters(**fronters_json)
            fronters.json = fronters_json
            self._fronters = fronters

            self.fronters_cache_timestamp = time.time()  # Update the cache timestamp
            log.info(f"Updated fronters")
...
class Member:
    ...
    @staticmethod
    def get_by_hid(hid, authorization=None):
        member_json = api_get(f"/m/{hid}")
        member = Member(**member_json)
        member.api_token = authorization
        return member
...
if __name__ == '__main__':
    ...
    test_sys = System.get_by_hid("TEST_SYS_ID", authorization=pk_token)
    test_sys_front = Fronters.get_by_hid("TEST_SYS_ID", authorization=pk_token)
